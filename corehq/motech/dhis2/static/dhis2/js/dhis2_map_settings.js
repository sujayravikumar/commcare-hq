...
hqDefine('dhis2/js/dhis2_map_settings', function () {
    ...
    module.Dhis2MapSettings = function (dataSetMaps, sendDataUrl, dataSets) {
        var self = this;
        ...
        self.init = function () {
            if (dataSetMaps.length > 0) {
                for (var i = 0; i < dataSetMaps.length; i++) {
                    var dataSetMap = new DataSetMap(Object.assign(dataSetMaps[i], {"dataSets": dataSets}));
                    dataSetMap.init();
        ...
        self.addDataSetMap = function () {
            self.dataSetMaps.push(new DataSetMap({"dataSets": dataSets}));
        };
